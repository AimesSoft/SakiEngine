...
- 支持 Steam 成就相关 API：`requestCurrentStats` / `getAchievement` / `setAchievement` / `clearAchievement` / `storeStats`。
...
Future<void> setupSteam() async {
  ...
  // 建议在成就操作前先调用一次
  await steamworks.requestCurrentStats();

  // 解锁成就并写回 Steam
  await steamworks.setAchievement('FIRST_STEP');
  await steamworks.storeStats();
}
```
...
## 成就接口说明
- `requestCurrentStats()`: 标记当前统计上下文可用（建议初始化成功后调用一次）。
- `getAchievement(achievementId)`: 查询成就是否已解锁，返回 `true/false`。
- `setAchievement(achievementId)`: 触发解锁成就（仅设置本地状态）。
- `clearAchievement(achievementId)`: 取消/重置某个成就（仅设置本地状态）。
- `storeStats()`: 将本地成就/统计变更提交给 Steam。

成就流程建议：
1. `initSteam(appId)`
2. `requestCurrentStats()`
3. `setAchievement(...)` 或 `clearAchievement(...)`
4. `storeStats()`
