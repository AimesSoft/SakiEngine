...
using SteamUserStatsRequestCurrentStatsFn = bool (*)(void *);
...
void *GetSteamUserStatsInterface() {
  ...
  auto steam_user_stats_fn =
      reinterpret_cast<SteamUserStatsFn>(dlsym(runtime.handle, "SteamAPI_SteamUserStats_v013"));
...
bool HandleRequestCurrentStats() {
  return EnsureSteamStatsReady();
}
...
bool HandleGetAchievement(const std::string &achievement_id, bool *unlocked) {
  ...
  auto *user_stats = GetSteamUserStatsInterface();
  if (user_stats == nullptr) {
    return false;
  }
  auto fn = reinterpret_cast<SteamUserStatsGetAchievementFn>(
      dlsym(GetSteamRuntime().handle, "SteamAPI_ISteamUserStats_GetAchievement"));
  if (fn == nullptr) {
    return false;
  }
  bool achieved = false;
...
bool HandleSetAchievement(const std::string &achievement_id) {
  ...
  auto fn = reinterpret_cast<SteamUserStatsSetAchievementFn>(
      dlsym(GetSteamRuntime().handle, "SteamAPI_ISteamUserStats_SetAchievement"));
  if (fn == nullptr) {
    return false;
  }
...
bool HandleClearAchievement(const std::string &achievement_id) {
  ...
  auto fn = reinterpret_cast<SteamUserStatsClearAchievementFn>(
      dlsym(GetSteamRuntime().handle, "SteamAPI_ISteamUserStats_ClearAchievement"));
  if (fn == nullptr) {
    return false;
  }
  return fn(user_stats, achievement_id.c_str());
...
bool HandleStoreStats() {
  ...
  auto fn = reinterpret_cast<SteamUserStatsStoreStatsFn>(
      dlsym(GetSteamRuntime().handle, "SteamAPI_ISteamUserStats_StoreStats"));
  if (fn == nullptr) {
    return false;
  }
...
bool ExtractAchievementId(FlMethodCall *method_call, std::string *out_id) {
  FlValue *arguments = fl_method_call_get_args(method_call);
  if (arguments == nullptr || fl_value_get_type(arguments) != FL_VALUE_TYPE_MAP) {
    return false;
  }
  FlValue *achievement_id_value = fl_value_lookup_string(arguments, "achievementId");
  if (achievement_id_value == nullptr || fl_value_get_type(achievement_id_value) != FL_VALUE_TYPE_STRING) {
    return false;
  }
  const gchar *value = fl_value_get_string(achievement_id_value);
  if (value == nullptr || *value == '\0') {
    return false;
  }
  *out_id = value;
...
static void flutter_steamworks_plugin_handle_method_call(
  ...
  if (strcmp(method, "getPlatformVersion") == 0) {
  ...
  } else if (strcmp(method, "requestCurrentStats") == 0) {
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_bool(HandleRequestCurrentStats())));
  } else if (strcmp(method, "getAchievement") == 0) {
    std::string achievement_id;
    if (!ExtractAchievementId(method_call, &achievement_id)) {
      response = FL_METHOD_RESPONSE(fl_method_error_response_new(
          "invalid-argument", "achievementId is required", nullptr));
    } else {
      bool unlocked = false;
      const bool ok = HandleGetAchievement(achievement_id, &unlocked);
      response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_bool(ok ? unlocked : FALSE)));
    }
    ...
    if (!ExtractAchievementId(method_call, &achievement_id)) {
      ...
      response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_bool(HandleSetAchievement(achievement_id))));
    }
    ...
    if (!ExtractAchievementId(method_call, &achievement_id)) {
      ...
      response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_bool(HandleClearAchievement(achievement_id))));
    }
  } else if (strcmp(method, "storeStats") == 0) {
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_bool(HandleStoreStats())));
  } else {
