...
class _NvlScreenState extends State<NvlScreen>
    ...
    implements NvlScreenController {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        // 使用推进管理器统一处理对话推进
        widget.progressionManager?.progressDialogue(source: 'nvl_tap');
      },
      child: FadeTransition(
        ...
        child: Stack(
          children: [
            ...
            if (widget.isMovieMode && _showCinematicBars)
              ..._buildCinematicBars(context),
  ...
  Widget _buildNvlDialogue(NvlDialogue dialogue, SakiEngineConfig config,
      double textScale, double uiScale, int index) {
    // 构建统一格式的文本
    String displayText;
    if (dialogue.speaker != null) {
      // 有说话人：角色名: "对话内容"
      displayText =
          '${dialogue.speaker}: ${dialogue.dialogue}'; //写给ai的话。不要把中文双引号改成英文双引号啊 显示出来很突兀
    } else {
      // 无说话人：直接显示内容（内心想法/旁白）
                    ...
                    text: TextSpan(
                      children: RichTextParser.createTextSpans(
                        ...
                        config.dialogueTextStyle.copyWith(
                          fontSize:
                              config.dialogueTextStyle.fontSize! * textScale,
                          color: Colors.white,
                          height: 1.6,
                          letterSpacing: 0.3,
                          fontFamily: config.dialogueFontFamily,
  ...
  Widget _buildContent(
      SakiEngineConfig config, double textScale, double uiScale) {
                ...
                child: Column(
                  ...
                  children: widget.nvlDialogues.asMap().entries.map((entry) {
                    ...
                    return _buildNvlDialogue(
                        dialogue, config, textScale, uiScale, index);
                  }).toList(),
                ),
              ),
            ),
