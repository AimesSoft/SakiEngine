...
class GameRenderer {
  ...
  static Future<void> drawBackground(
    ...
    if (ColorBackgroundRenderer.isValidHexColor(backgroundName)) {
      ColorBackgroundRenderer.drawColorBackground(
          canvas, backgroundName, canvasSize,
    ...
    try {
      ...
      if (CgImageCompositor().isCachePath(backgroundName)) {
        ...
        // 常规资源路径处理
        backgroundPath = await AssetManager()
            .findAsset('backgrounds/${backgroundName.replaceAll(' ', '-')}');
      }
  ...
  static Future<void> drawCharacters(
    ...
    for (final entry in characters.entries) {
      ...
      await drawSingleCharacter(canvas, characterId, characterState,
          distributedPoseConfigs, canvasSize);
    }
  }
  ...
  static Future<void> drawCgCharacters(
    ...
    for (final entry in charactersByResourceId.values) {
      final characterId = entry.key;
      final characterState = entry.value;
      await drawSingleCgCharacter(
          canvas, characterId, characterState, canvasSize);
    }
  }
  ...
  static Future<void> drawSingleCgCharacter(
    ...
    try {
      ...
      for (final layerInfo in layerInfos) {
        ...
        if (image != null) {
          _drawCgCharacterLayer(
              canvas, image, canvasSize, characterState.animationProperties);
        }
      }
    } catch (e) {
  ...
  static void _drawCgCharacterLayer(
    ...
    if (animationProperties != null && animationProperties.isNotEmpty) {
      ...
      // 应用平移
      final xOffset =
          (animationProperties['xcenter'] ?? 0.0) * canvasSize.width;
      final yOffset =
          (animationProperties['ycenter'] ?? 0.0) * canvasSize.height;
    ...
    try {
      ...
      for (int i = 0; i < layerImages.length; i++) {
        ...
        final adjustedParams =
            _applyExpressionOffset(renderParams, xOffset, yOffset, scale);
  ...
  static CharacterRenderParams _calculateCharacterRenderParams(
    ...
    final imageSize =
        Size(sampleImage.width.toDouble(), sampleImage.height.toDouble());
    Size paintSize;
  ...
  /// 在Canvas上绘制角色图层
  static void _drawCharacterLayer(
      Canvas canvas, ui.Image image, CharacterRenderParams params,
      [double? expressionAlpha]) {
  ...
  static void _drawImageWithBoxFitCover(
      Canvas canvas, ui.Image image, Size canvasSize,
      {Map<String, double>? animationProperties}) {
    final imageAspectRatio = image.width / image.height;
    final canvasAspectRatio = canvasSize.width / canvasSize.height;
