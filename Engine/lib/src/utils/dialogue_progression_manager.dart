...
class DialogueProgressionManager {
  ...
  bool Function()? externalTypewriterComplete;
  VoidCallback? finishExternalTypewriter;
  ...
  /// Returns whether a request was dispatched to the story executor.
  bool progressDialogue({bool isAutomated = false}) {
    if (externalTypewriterComplete?.call() == false) {
      if (!isAutomated) {
        finishExternalTypewriter?.call();
    ...
    final recentlySkippedTypewriter =
        ...
        now.difference(_lastTypewriterSkipAt!) <= _rapidProgressGuardDuration &&
  ...
  void _markCurrentDialogueAsRead() {
    ...
    if (currentState.dialogue != null &&
        currentState.dialogue!.trim().isNotEmpty) {
    ...
    if (currentState.isNvlMode ||
        currentState.isNvlMovieMode ||
        currentState.isNvlnMode) {
  ...
  bool get canProgressDirectly {
    if (externalTypewriterComplete != null) {
      return externalTypewriterComplete!();
    }
